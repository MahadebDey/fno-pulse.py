...
with st.sidebar:
    ...
    if dhan_client_id and dhan_token and DHAN_AVAILABLE:
        try:
            ...
            st.session_state["dhan_client_id"] = dhan_client_id
            st.session_state["dhan_token"] = dhan_token
            st.success("🟢 Dhan API Live Connected")
        except Exception:
            st.error("Dhan Connection Error")
            dhan_instance = None
    else:
...
with st.expander("⚡ DHAN ADVANCED OPTIONS & EQUITY TRADING TERMINAL", expanded=True):
    if not dhan_instance:
        ...
        # Check Total Unrealized Loss & Kill Switch
        total_pnl = 0.0
        try:
        ...
        # TAB 1: Order Placement (Pure English Labels)
        with tab_order:
                    ...
                    with st.spinner("Submitting order sequence to DhanHQ API..."):
                        try:
                            ...
                            if entry_resp.get("status") == "success":
                                ...
                                # Secondary Auto SL-Limit Exit Order Placement
                                if enable_sl_target:
        ...
        # TAB 2: Live Positions (All English Columns & Labels)
        with tab_pos:
            ...
            try:
                ...
                if positions_resp and positions_resp.get("status") == "success" and positions_resp.get("data"):
                    ...
                    if open_positions:
                        ...
                        st.write("---")
                        # Dedicated Limit / Market Exit for individual open position
